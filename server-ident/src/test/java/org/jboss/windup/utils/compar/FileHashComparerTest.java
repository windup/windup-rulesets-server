...
public class FileHashComparerTest {
    ...
    public void testCompareHashesAndDir() throws Exception {
        ...
        assertEquals(FileHashComparer.MatchResult.MATCH,    results.getMatches().get(Paths.get(STR_ORACLE_DS)));
        assertEquals(FileHashComparer.MatchResult.MISMATCH, results.getMatches().get(Paths.get(STR_DATASOURCES)));
        assertEquals(FileHashComparer.MatchResult.MISSING,  results.getMatches().get(Paths.get(STR_NON_EXISTENT)));
    }
